...
func seedPatrulheiroSkills(db *gorm.DB) {
	...
	skills := []domain.Skill{
		...
		// ── NÍVEL 1 — DIÁRIO ────────────────────────────────────────
		{
			Name: "Golpe dos Dois Lobos", Edition: "4e", ClassID: &id,
			Description: "Você ataca de dois lados ao mesmo tempo, prendendo o inimigo no lugar com suas duas lâminas.",
			Keywords:   "Arma, Marcial",
			ActionType: "Ação Padrão", Range: "Arma corpo a corpo",
			Target: "Uma criatura",
			Attack: "Força vs. CA, dois ataques",
			Hit:    "2[A] + mod Força de dano por ataque. Se ambos os ataques acertarem, o alvo fica imobilizado (TR encerra).",
			Miss:   "Metade do dano por ataque que acertar.",
			PowerType: domain.PowerDaily, Level: 1,
		},
		{
			Name: "Exploração Furtiva", Edition: "4e", ClassID: &id,
			Description: "Com um ataque preciso e poderoso, você explora uma brecha nas defesas do inimigo causando dano devastador.",
			Keywords:   "Arma, Marcial",
			ActionType: "Ação Padrão", Range: "Arma corpo a corpo ou à distância",
			Target: "Uma criatura",
			Attack: "Força vs. CA (corpo a corpo) ou Destreza vs. CA (à distância)",
			Hit:    "3[A] + mod Força (corpo a corpo) ou 3[A] + mod Destreza (à distância) de dano.",
			Miss:   "Metade do dano.",
			PowerType: domain.PowerDaily, Level: 1,
		},
