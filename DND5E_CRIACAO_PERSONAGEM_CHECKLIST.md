...
- Bônus de atributo de raça/antecedente ausente (Seção 0/2) — formalizar como 2014, 2024, ou manter como está (simplificação já documentada)?
- ASI vs. Feat na mesma escolha (Seção 2) — vale impedir pegar os dois no mesmo nível?
- Subclasses inconsistentes entre classes — Mago e Guerreiro sem subclasse nenhuma, as demais todas seedadas no nível 1 mesmo quando o RAW manda nível diferente (Seção 1) — vale corrigir o nível certo por classe, ou é aceitável simplificar todas pro nível 1?
- `domain.Background` parece uma tabela morta (Seção 1) — remover, ou existe um uso que não encontrei?
